...
console.log(alternateRicks.split("Rick").length);

// Q4
// How many characters are there in rickSaying (without spaces)? 
console.log(rickSaying.split(" ").length);
...
let decode=secret;
notCode.forEach ( function (x) {
    decode = decode.replace(x, "");
    return (decode);
})
console.log(decode);
...
let realE = enemies.replace("Scary Terry, ", "");
console.log(realE);
